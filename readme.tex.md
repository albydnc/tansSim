...
Given a particles' array and a rms angle value, a rotation matrix is built with the particle coordinates and then a scattering vector is computed via random generation of $\theta$ and $\phi$ values. The two are multiplied and the result are the new particle coordinates, which replace the originals in the array.
